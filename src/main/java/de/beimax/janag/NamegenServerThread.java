...
public class NamegenServerThread extends Thread {
    ...
    /**
     * @param cs socket of worker Constructor
     * @param ng name generator instance
     */
    ...
    private String[] getRandomNames(StreamTokenizer st) throws IOException {
        ...
        if (st.sval == null || st.sval.equals("")) {
            throw new IOException("Argument 2 must be a pattern name enclosed by \""); //$NON-NLS-2$
        }
        ...
        st.nextToken();
        if (st.sval == null || st.sval.equals("")) {
            throw new IOException("Argument 3 must be a gender name enclosed by \""); //$NON-NLS-2$
        }
